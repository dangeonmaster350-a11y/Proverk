...
const DB = {
  users: [
    { id:"u1", username:"alexey_k",  fullName:"Алексей Козлов",  avatar:null, phone:"79991112233", hidePhone:false, bio:"Люблю кофе и код ☕",     online:true,  lastSeen:null,              premium:true,  role:"user",  banned:false, verified:true,  major:false },
    { id:"u2", username:"marina_v",  fullName:"Марина Волкова",  avatar:null, phone:"79994445566", hidePhone:true,  bio:"Дизайнер 🎨",             online:false, lastSeen:Date.now()-7*60000,  premium:false, role:"user",  banned:false, verified:false, major:false },
    { id:"u3", username:"dmitry_p",  fullName:"Дмитрий Петров",  avatar:null, phone:"79997778899", hidePhone:false, bio:"",                        online:false, lastSeen:Date.now()-2*3600000,premium:false, role:"user",  banned:false, verified:false, major:false },
    { id:"u4", username:"anon_7777", fullName:"Аноним #7777",    avatar:null, phone:"8881234567",  hidePhone:true,  bio:"👤 Анонимный аккаунт",    online:true,  lastSeen:null,              premium:true,  role:"user",  banned:false, verified:false, major:true  },
    { id:"u5", username:"anon_9999", fullName:"Аноним #9999",    avatar:null, phone:"8889999888",  hidePhone:true,  bio:"👤 Анонимный аккаунт",    online:false, lastSeen:Date.now()-30*60000, premium:true,  role:"user",  banned:false, verified:false, major:false },
  ],
  messages: {
...
const OWNER_USER = {
  ...
  premium:true, role:"owner", banned:false, verified:true, major:true,
};
...
const LS_DB  = "volt_db_v2";
const LS_SES = "volt_session_v2";

function saveDB() {
  try {
    // Don't persist lastSeen timestamps as Date.now() offsets — save as-is
    localStorage.setItem(LS_DB, JSON.stringify({
      ...
      registeredPhones: DB.registeredPhones,
    }));
...
function loadDB() {
  try {
    ...
    if (saved.favorites)        DB.favorites        = saved.favorites;
    if (saved.muted)            DB.muted            = saved.muted;
    if (saved.blocked)          DB.blocked          = saved.blocked;
    if (saved.archived)         DB.archived         = saved.archived;
    if (saved.registeredPhones) DB.registeredPhones = saved.registeredPhones;
  } catch(e) { console.warn("loadDB failed", e); }
}

function saveSession(user) {
  try {
    if (user) localStorage.setItem(LS_SES, JSON.stringify({ userId: user.id, phone: user.phone }));
...
// Patch render() to auto-save DB on every render
const _origRender_ref = { fn: null };
...
function badgesHtml(user){
  let b="";
  if(user.verified)b+=`<span class="badge-verified" title="Верифицирован">✔</span>`;
  if(user.major)b+=`<span class="badge-major" title="Мажор 💎">💎</span>`;
...
function doUsernameRegister(){
  ...
  captchaDone=false;
  doLogin({id:"me",username,fullName,avatar:null,phone:"",hidePhone:false,bio:"",online:true,lastSeen:null,premium:false,role:"user",banned:false,verified:false,major:false});
}
...
function doPhoneRegister(){
  ...
  const newUser={
    ...
    online:true,lastSeen:null,premium:isAnon||isOwner,role:isOwner?"owner":"user",banned:false,verified:false,major:false,
  };
...
function toggleFav(id){const i=DB.favorites.indexOf(id);if(i>=0)DB.favorites.splice(i,1);else DB.favorites.push(id);saveDB();render();}
  ...
  return `<div style="padding-bottom:24px;">
    ...
    <div class="profile-center">
      ...
      <div style="text-align:center;margin-top:16px;width:100%;">
        ...
        ${u.premium?`<div style="margin-top:12px;padding:0 16px;">${premiumBlockHtml()}</div>`:`<button style="margin-top:12px;padding:10px 24px;background:linear-gradient(135deg,#f59e0b,#ef8c02);border:none;border-radius:12px;color:#fff;font-weight:700;font-size:13px;cursor:pointer;" onclick="AppState.screen='buypremium';render()">⭐ Купить Volt Premium</button>`}
...
function saveProfile(){
  ...
  DB.users.forEach(u=>{if(u.id===AppState.currentUser.id){u.fullName=name;u.username=username;u.bio=bio;}});
...
function renderSettingsTab(){
  ...
  return `<div style="padding-bottom:24px;">
    ...
    <div style="padding:0 16px;display:flex;flex-direction:column;gap:8px;"><button class="logout-btn" onclick="handleLogout()">Выйти из аккаунта</button><button class="logout-btn" style="background:rgba(239,68,68,0.07);color:#ef4444;border-color:rgba(239,68,68,0.2);font-size:12px;" onclick="if(confirm('Сбросить все данные и аккаунты?')){\localStorage.clear();location.reload();}">🗑 Сбросить все данные</button></div>
  </div>`;
}
...
function toggleMute(id){const i=DB.muted.indexOf(id);if(i>=0)DB.muted.splice(i,1);else DB.muted.push(id);saveDB();render();}
function toggleBlock(id){const i=DB.blocked.indexOf(id);if(i>=0)DB.blocked.splice(i,1);else DB.blocked.push(id);saveDB();render();}
function toggleArchive(id){const i=DB.archived.indexOf(id);if(i>=0)DB.archived.splice(i,1);else DB.archived.push(id);saveDB();render();}
...
function renderAdminPanel(){
  const tabs=[["users","👥","Юзеры"],["stats","📊","Статистика"],["add","➕","Добавить"],["broadcast","📢","Рассылка"],["logs","📋","Логи"],["premshop","🛒","Premium"]];
  ...
  else if(adminTab==="premshop")content=renderAdminPremShop();
  ...
  return filtered.map(u=>`
    <div class="user-card${u.banned?" banned":""}${adminEditUser?.id===u.id?" editing":""}">
      ...
      ${adminEditUser?.id===u.id?`<div class="action-btns">
        ...
        <button class="action-btn${u.verified?" verified":""}" onclick="adminToggleVerified('${u.id}')">${u.verified?"✔ Снять галочку":"✔ Верифицировать"}</button>
        <button class="action-btn${u.major?" major":""}" onclick="adminToggleMajor('${u.id}')">${u.major?"💎 Снять мажора":"💎 Выдать мажора"}</button>
...
function adminTogglePremium(id){DB.users.forEach(u=>{if(u.id===id)u.premium=!u.premium;});addAdminLog("Premium toggle",DB.users.find(u=>u.id===id)?.username||id);saveDB();renderAdminUsersInPlace();}
function adminToggleVerified(id){DB.users.forEach(u=>{if(u.id===id)u.verified=!u.verified;});addAdminLog(DB.users.find(u=>u.id===id)?.verified?"Верификация выдана":"Верификация снята",DB.users.find(u=>u.id===id)?.username||id);saveDB();renderAdminUsersInPlace();}
function adminToggleMajor(id){DB.users.forEach(u=>{if(u.id===id)u.major=!u.major;});addAdminLog(DB.users.find(u=>u.id===id)?.major?"Мажор выдан":"Мажор снят",DB.users.find(u=>u.id===id)?.username||id);saveDB();renderAdminUsersInPlace();}
function adminToggleBan(id){DB.users.forEach(u=>{if(u.id===id)u.banned=!u.banned;});addAdminLog(DB.users.find(u=>u.id===id)?.banned?"Бан":"Разбан",DB.users.find(u=>u.id===id)?.username||id);saveDB();renderAdminUsersInPlace();}
function adminDeleteUser(id){if(!confirm("Удалить пользователя?"))return;const idx=DB.users.findIndex(u=>u.id===id);if(idx>=0){addAdminLog("Удалён",DB.users[idx].username);DB.users.splice(idx,1);}adminEditUser=null;saveDB();renderAdminUsersInPlace();}
...
function adminAddAnon(){
  ...
  const newId="u"+Date.now();
  const anonNum=d.slice(-4);
  const nu={id:newId,username:"anon_"+anonNum,fullName:"Аноним #"+anonNum,avatar:null,phone:d,hidePhone:true,bio:"👤 Анонимный аккаунт",online:false,lastSeen:null,premium:true,role:"user",banned:false,verified:false,major:false};
...
// ─── Admin: Premium Shop ─────────────────────────────────────────────────────
let premShopGiftUser = "", premShopSuccess = "";
const PREM_PLANS = [
  {id:"1m", label:"1 месяц",  price:"199 ₽",  desc:"Базовый план",     color:"#4f7cff"},
  {id:"3m", label:"3 месяца", price:"399 ₽",  desc:"Экономия 200 ₽",   color:"#43c59e"},
  {id:"6m", label:"6 месяцев",price:"499 ₽",  desc:"Экономия 695 ₽",   color:"#a78bfa"},
  {id:"12m",label:"12 месяцев",price:"699 ₽", desc:"Выгоднее всего!",  color:"#f59e0b"},
];
function renderAdminPremShop(){
  const q=premShopGiftUser.toLowerCase();
  const filteredU=q?DB.users.filter(u=>u.fullName.toLowerCase().includes(q)||u.username.toLowerCase().includes(q)):[];
  return `<div style="padding:16px;display:flex;flex-direction:column;gap:16px;">
    <div style="background:linear-gradient(135deg,rgba(79,124,255,0.13),rgba(124,79,255,0.13));border-radius:16px;padding:16px;border:1px solid rgba(79,124,255,0.2);">
      <div style="font-size:15px;font-weight:700;color:var(--text);margin-bottom:4px;">⭐ Volt Premium</div>
      <div style="font-size:12px;color:var(--sub);">Купите Premium пользователю или продайте через профиль</div>
    </div>
    <div style="font-size:12px;color:var(--sub);text-transform:uppercase;letter-spacing:1px;font-weight:600;">Тарифы</div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
      ${PREM_PLANS.map(p=>`<div style="background:rgba(255,255,255,0.04);border:1px solid ${p.color}33;border-radius:14px;padding:14px;text-align:center;">
        <div style="font-size:18px;font-weight:800;color:${p.color};">${p.price}</div>
        <div style="font-size:13px;font-weight:600;color:var(--text);margin:4px 0;">${p.label}</div>
        <div style="font-size:11px;color:var(--sub);">${p.desc}</div>
      </div>`).join("")}
    </div>
    <div style="font-size:12px;color:var(--sub);text-transform:uppercase;letter-spacing:1px;font-weight:600;">Подарить пользователю</div>
    <input class="inp" id="premshop-user-inp" value="${escHtml(premShopGiftUser)}" placeholder="Поиск пользователя..." oninput="premShopGiftUser=this.value;renderAdminPremShopInPlace()">
    ${filteredU.length>0?`<div style="border:1px solid var(--divider);border-radius:12px;overflow:hidden;">${filteredU.slice(0,5).map(u=>`<div style="padding:10px 14px;display:flex;align-items:center;gap:10px;border-bottom:1px solid var(--divider);">
      ${avatarHtml(u,34,false)}
      <div style="flex:1;min-width:0;"><div style="font-size:13px;font-weight:600;color:var(--text);display:flex;align-items:center;gap:4px;">${escHtml(u.fullName)}${badgesHtml(u)}</div><div style="font-size:11px;color:var(--sub);">@${escHtml(u.username)}</div></div>
      <div style="display:flex;flex-direction:column;gap:4px;">
        ${PREM_PLANS.map(p=>`<button style="font-size:10px;padding:3px 8px;background:${p.color}22;color:${p.color};border:1px solid ${p.color}44;border-radius:6px;cursor:pointer;font-weight:600;" onclick="adminGiftPremium('${u.id}','${p.label}')">${p.price}</button>`).join("")}
      </div>
    </div>`).join("")}</div>`:""}
    ${premShopSuccess?`<div style="background:rgba(34,197,94,0.13);border:1px solid rgba(34,197,94,0.3);border-radius:12px;padding:12px;text-align:center;color:#22c55e;font-weight:600;">✓ ${escHtml(premShopSuccess)}</div>`:""}
    <div style="background:rgba(245,158,11,0.08);border:1px solid rgba(245,158,11,0.2);border-radius:12px;padding:12px;font-size:11px;color:var(--sub);">
      💡 Пользователи также могут купить Premium самостоятельно через свой профиль
    </div>
  </div>`;
}
function renderAdminPremShopInPlace(){
  const el=document.getElementById("app");
  if(adminTab==="premshop")render();
}
function adminGiftPremium(userId,planLabel){
  DB.users.forEach(u=>{if(u.id===userId)u.premium=true;});
  const uname=DB.users.find(u=>u.id===userId)?.username||userId;
  addAdminLog("Premium подарен ("+planLabel+")",uname);
  saveDB();
  premShopSuccess="Premium "+planLabel+" выдан @"+uname+"!";
  premShopGiftUser="";
  render();
  setTimeout(()=>{premShopSuccess="";},3000);
}
...
// ─── Buy Premium Screen ───────────────────────────────────────────────────────
let buyPremSelected = null, buyPremResult = "";
const BUY_PLANS = [
  {id:"1m",  label:"1 месяц",   price:"199 ₽",  priceNum:199,  saved:"",           color:"#4f7cff", popular:false},
  {id:"3m",  label:"3 месяца",  price:"399 ₽",  priceNum:399,  saved:"Экономия 200 ₽", color:"#43c59e", popular:false},
  {id:"6m",  label:"6 месяцев", price:"499 ₽",  priceNum:499,  saved:"Экономия 695 ₽", color:"#a78bfa", popular:true},
  {id:"12m", label:"12 месяцев",price:"699 ₽",  priceNum:699,  saved:"Выгоднее всего!", color:"#f59e0b", popular:false},
];
function renderBuyPremiumScreen(){
  return `<div style="min-height:100vh;background:var(--bg);display:flex;flex-direction:column;">
    <div style="display:flex;align-items:center;gap:12px;padding:16px;border-bottom:1px solid var(--divider);">
      <button class="back-btn" onclick="AppState.screen='app';render()">←</button>
      <div style="font-size:16px;font-weight:700;color:var(--text);">⭐ Volt Premium</div>
    </div>
    <div style="padding:20px 16px 0;text-align:center;">
      <div style="font-size:36px;margin-bottom:8px;">⭐</div>
      <div style="font-size:20px;font-weight:800;color:var(--text);margin-bottom:6px;">Volt Premium</div>
      <div style="font-size:13px;color:var(--sub);line-height:1.6;">Цветной ник · Анонимный номер · Реакции · Без лимитов на сообщения</div>
    </div>
    <div style="padding:20px 16px;display:flex;flex-direction:column;gap:10px;">
      ${BUY_PLANS.map(p=>`<div onclick="buyPremSelected='${p.id}';renderBuyPremInPlace()" style="position:relative;background:${buyPremSelected===p.id?`${p.color}22`:`rgba(255,255,255,0.04)`};border:2px solid ${buyPremSelected===p.id?p.color:`var(--divider)`};border-radius:16px;padding:16px;cursor:pointer;transition:all 0.15s;">
        ${p.popular?`<div style="position:absolute;top:-10px;right:14px;background:${p.color};color:#fff;font-size:10px;font-weight:700;padding:2px 10px;border-radius:20px;">🔥 ПОПУЛЯРНО</div>`:""}
        <div style="display:flex;align-items:center;justify-content:space-between;">
          <div>
            <div style="font-size:15px;font-weight:700;color:var(--text);">${p.label}</div>
            ${p.saved?`<div style="font-size:11px;color:${p.color};font-weight:600;margin-top:2px;">${p.saved}</div>`:""}
          </div>
          <div style="text-align:right;">
            <div style="font-size:20px;font-weight:800;color:${p.color};">${p.price}</div>
            <div style="font-size:10px;color:var(--sub);">разовый платёж</div>
          </div>
        </div>
        ${buyPremSelected===p.id?`<div style="margin-top:2px;font-size:11px;color:${p.color};">● Выбрано</div>`:""}
      </div>`).join("")}
    </div>
    ${buyPremResult?`<div style="margin:0 16px;padding:14px;background:rgba(34,197,94,0.13);border:1px solid rgba(34,197,94,0.3);border-radius:12px;text-align:center;color:#22c55e;font-weight:700;font-size:14px;">${escHtml(buyPremResult)}</div>`:`<div style="padding:0 16px;"><button style="width:100%;padding:15px;background:${buyPremSelected?`linear-gradient(135deg,#f59e0b,#ef8c02)`:`var(--divider)`};color:${buyPremSelected?"#fff":"var(--sub)"};border:none;border-radius:14px;font-size:15px;font-weight:700;cursor:${buyPremSelected?"pointer":"default"};transition:all 0.2s;" onclick="doBuyPremium()">
      ${buyPremSelected?"⭐ Оплатить "+BUY_PLANS.find(p=>p.id===buyPremSelected)?.price:"Выберите тариф"}
    </button></div>`}
    <div style="padding:16px;margin-top:auto;">
      <div style="font-size:11px;color:var(--sub);text-align:center;line-height:1.6;">Оплата через защищённый шлюз. Автопродление не подключается. Возврат в течение 24 часов.</div>
    </div>
  </div>`;
}
function renderBuyPremInPlace(){
  const el = document.getElementById("app");
  if(AppState.screen==="buypremium") el.innerHTML = renderBuyPremiumScreen();
}
function doBuyPremium(){
  if(!buyPremSelected) return;
  const plan = BUY_PLANS.find(p=>p.id===buyPremSelected);
  if(!plan) return;
  // Simulate purchase: activate premium
  AppState.currentUser = {...AppState.currentUser, premium: true};
  const u = DB.users.find(u=>u.id===AppState.currentUser.id);
  if(u) u.premium = true;
  saveDB();
  buyPremResult = "✓ Premium на "+plan.label+" активирован!";
  render();
  setTimeout(()=>{buyPremResult="";buyPremSelected=null;AppState.screen="app";AppState.tab="profile";render();}, 2000);
}
...
function render(){
  ...
  else if(AppState.screen==="buypremium") app.innerHTML=renderBuyPremiumScreen();
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  if(ses){
    let user = null;
    if(ses.userId === "owner") user = {...OWNER_USER};
    else user = DB.users.find(u=>u.id===ses.userId) || DB.users.find(u=>u.phone===ses.phone);
    if(user && !user.banned){
      AppState.currentUser = {...user};
      if(!DB.messages[user.id]) DB.messages[user.id]=[];
      AppState.screen = "app";
